...
rule featureCounts:
    ...
    shell:
        """
        echo -e '{params.software} -p -T {params.threads} -t {params.feature_name} -g {params.meta_feature} -o {output.count_file} -a {params.ref_file} {input.bam_sorted_file}' > {params.shell_script}
        
        {params.software} -p -T {params.threads} -t {params.feature_name} -g {params.meta_feature} -o {output.count_file} -a {params.ref_file} {input.bam_sorted_file} >& {log.log_file}
