...
#'
geom_venn2 <- function(setlist, xlim=c(0,1), ylim=c(0,1), numbersize=4, numbercolor="black", textsize=4, textcolor="black", fillCol=TRUE, borderCol=FALSE, fixedCoords=TRUE, ...){
...
setwd("D:/SijianXia/08Shenzhenbay_lab/06_my_script/R/ggvenn2/")

source("./findOverlap.R")
set1 <- rownames(subset(mtcars, mpg>18))
set2 <- rownames(subset(mtcars, qsec>18))
set3 <- rownames(subset(mtcars, cyl<5))
...
ggplot() + geom_venn2(carset,textcolor = "black",
                     alpha=0.65,borderCol = F)
